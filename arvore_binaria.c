...
void criar_arvore_binaria_externa(FILE *arq_entrada, FILE *arq_saida)
{
    ...
    // printf("\n\nDeseja buscar no arquivo externo da arvore binaria uma chave? Informe o elemento.");
    printf("\n\nInforme o elemento que deseja buscar (ou -1, caso nao deseje realizar uma busca): ");
    scanf("%d", &elemento);
    ...
    if(elemento != -1)
    {
        ...
        if (f != NULL)
        {
            printf("\nFuncionario %s de codigo %d encontrado! \n", f->nome, f->cod);
        }
        else
        {
